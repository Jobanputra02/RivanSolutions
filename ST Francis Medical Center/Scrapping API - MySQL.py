...
mydb = mysql.connector.connect(
    host="localhost",
    user="root",
    password="Nilkanth@78",
    database="steerhealth"
)
mycursor = mydb.cursor()

create_database = "CREATE DATABASE steerhealth"
mycursor.execute(create_database)


# Create Table
mycursor.execute("DROP TABLE IF EXISTS Doctor_Data;")
create_table = '''CREATE TABLE Doctor_Data ( Full_Name VARCHAR(255), Specialty VARCHAR(255), Add_Specialty VARCHAR(255), Gender VARCHAR(255), Practice VARCHAR(255), Address VARCHAR(255), City VARCHAR(255), State VARCHAR(255), ZIP VARCHAR(255), Full_Address VARCHAR(255), Phone VARCHAR(255), Fax VARCHAR(255) )'''
...
for i in range(0, json_data['size']):
    ...
    insert_row = "INSERT INTO Doctor_Data (Full_Name, Specialty, Add_Specialty, Gender, Practice, Address, City, State, ZIP, Full_Address, Phone, Fax) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    values_to_add = (full_name, speciality, additional_speciality, gender, practice, address, city, state, zipcode, full_address, phone, fax)
    mycursor.execute(insert_row, values_to_add)
    mydb.commit()
