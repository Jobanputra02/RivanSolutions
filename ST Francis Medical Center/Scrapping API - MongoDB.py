...
# Create collection
steerhealth_db = mongoclient["steerhealth"]
mycol = steerhealth_db["doctors"]
...
for i in range(0, json_data['size']):
    ...
    dict_to_add_in_mongodb = dict_to_add
    dict_to_add_in_mongodb['_id'] = i
    x = mycol.insert_one(dict_to_add_in_mongodb)

    df_to_add = pd.DataFrame(dict_to_add, index=[i])
